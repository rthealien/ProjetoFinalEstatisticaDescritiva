...
## Análise cruzada por raça/cor e renda familiar
...
microdados_musica_raca_renda = microdados_musica_sem_NA %>%
  select(raca, NT_GER, renda_familiar) %>%
  group_by(raca, renda_familiar) %>%
  summarise(
    quantidade = n(),
    media = mean(NT_GER, na.rm = T),
    mediana = median(NT_GER, na.rm = T),
...
microdados_musica_raca_renda %>%
  kbl %>%
  kable_material_dark(full_width = F)
#
#
#grafico_boxplot3 = ggplot(microdados_musica_sem_NA,
#                          aes(x = renda_familiar, y = NT_GER, fill =
#                                renda_familiar)) +
#  geom_boxplot() +
...
grafico_histograma2 = ggplot(microdados_musica_sem_NA, aes(x = NT_GER, fill = raca)) +
  geom_histogram() +
  ggtitle("Gráfico histograma da Nota por raça e renda familiar") +
  xlab("Notas") +
  ylab("Renda Familiar")
